...
# Runs Model 1, from homework last week
...
# Runs Model 1, like homework last week, but has bias term
def RunModel2(M, N, k, eta, reg, Y_train, Y_test, GraphFlag=True):
...
# Out of Box: Runs a model based of the package surprise for matrix factorization
def RunModel3(M, N, k, eta, reg, Y_train, Y_test):
    ...
    return e_in_3, e_out_3
        
# Out of Box: Runs a model based on scipy SVD for matrix factorization
def RunModel4(M, N, k, eta, reg, Y_train, Y_test):
...
if __name__ == "__main__":
    ...
    regLst = [0, 0.0001, 0.001, 0.01, 0.1, 1]
    etaLst = [0, 0.0001, 0.001, 0.01, 0.1, 1]
    ...
    bestErrOut1 = -1
    bestErrOut2 = -1
    print('Optimizing Eta and Reg for models 1 & 2...')
    for reg in regLst:
        for eta in etaLst:

            errIn1, errOut1 = RunModel1(M, N, k, eta, reg, Y_train, Y_test, False)
            errIn2, errOut2 = RunModel2(M, N, k, eta, reg, Y_train, Y_test, False)

            if errOut1 < bestErrOut1 or bestErrOut1 == -1:
                bestReg1 = reg
                bestEta1 = eta
                bestErrOut1 = errOut1

            if errOut2 < bestErrOut2 or bestErrOut2 == -1:
                bestReg2 = reg
                bestEta2 = eta
                bestErrOut2 = errOut2


    print('From', regLst, 'best reg for Model1', bestReg1)
    print('From', etaLst, 'best eta for Model1', bestEta1)
    print('Best errout:', bestErrOut1)

    print('From', regLst, 'best reg for Model2', bestReg2)
    print('From', etaLst, 'best eta for Model2', bestEta2)
    print('Best errout:', bestErrOut2)

    # Train our four models:
    RunModel1(M, N, k, bestEta1, bestReg1, Y_train, Y_test)
    RunModel2(M, N, k, bestEta2, bestReg2, Y_train, Y_test)
    RunModel3(M, N, k, eta, reg, Y_train, Y_test)
    RunModel4(M, N, k, eta, reg, Y_train, Y_test)
